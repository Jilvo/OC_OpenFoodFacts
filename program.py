...
class Program():
    ...
    def disp_sub_product(self):
        ...
        for row in record:
            ...
            self.cursor.execute(self.operation)
            record_sub_from = self.cursor.fetchall()
            for substituant in record_sub_from:
                print("le substituant choisi à pour id", substituant[0])
                print("le nom du substituant est :", substituant[1])
                print("le nutriscore est :", substituant[2])
                print("l'url est :", substituant[3])
                print("le magasin est :", substituant[4])
                print("l'id de la catégorie est :", substituant[5], '\n')
            # print("il est substituant de :", row[2])
            self.operation = ("SELECT * FROM `product` WHERE `id` = " + str(sub_to) + " ")
            self.cursor.execute(self.operation)
            record_sub_to = self.cursor.fetchall()
            for substitut in record_sub_to:
                print("le substitué choisi à pour id", substitut[0])
                print("le nom du substitué est :", substitut[1])
                print("le nutriscore est :", substitut[2])
                print("l'url est :", substitut[3])
                print("le magasin est :", substitut[4])
                print("l'id de la catégorie est :", substitut[5], '\n')
